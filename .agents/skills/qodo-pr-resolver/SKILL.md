...
- Your prior **fix-summary comments** (the marker-bearing comments above) — fixed/deferred issues with human-readable rationale text, machine-readable `file`, `line`, and optional `comment_id` location fields, plus separate machine-readable `decision` and direction-sensitive `action` fields. Parse `decision` as `fixed`, `deferred`, `held`, or `hard_stopped`; require a concrete transition when `decision=fixed`, and require `action=none` when `decision=deferred`, `held`, or `hard_stopped`. Validate this schema before repeat or contradiction tagging and match duplicate titles using the location fields rather than rationale text.
