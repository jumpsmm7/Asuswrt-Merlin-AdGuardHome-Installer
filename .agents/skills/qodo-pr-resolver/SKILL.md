...
- Your prior **fix-summary comments** (the marker-bearing comments above) — fixed/deferred issues with human-readable rationale text, plus separate machine-readable `decision` and direction-sensitive `action` fields. Parse `decision` as `fixed`, `deferred`, `held`, or `hard_stopped`; parse `action` as the concrete transition (for example `guard_added` or `guard_removed`) or `none` for deferred/held decisions. Validate this schema before repeat or contradiction tagging and keep both fields separate from rationale text.
