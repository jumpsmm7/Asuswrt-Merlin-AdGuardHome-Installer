#!/bin/sh
# Validate one or more .md5 files against their matching source files.
# BusyBox/ash-compatible.
#
# Usage:
#   sh tools/check-md5.sh                  # check all *.md5 files found in repo
#   sh tools/check-md5.sh installer.md5    # check one checksum file
#
# Mapping rule:
...
check_one() {
	...
	*)
	...
	_expected="$(awk '{print $1; exit}' "${_md5_file}")"
