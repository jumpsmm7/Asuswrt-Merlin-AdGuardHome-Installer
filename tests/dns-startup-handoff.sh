...
: >"${CALLS_FILE}"
printf '%s\n' 'ADGUARD_INSTALL_MODE="lan"' >"${WORK_DIR}/.config" || fail 'could not set LAN install mode'
DNSMASQ_RUNNING=0
DNS_STATE=free
...
[ ! -e "${DNS_HANDOFF_FILE}" ] || fail 'LAN no-handoff pre-start left a stale handoff marker'
