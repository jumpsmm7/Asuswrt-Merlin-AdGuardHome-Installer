...
DNS_STATE=busy_alt
if kill_dns_port_owners; then
	fail 'DNS owner cleanup accepted an unknown port 53 owner without force'
fi
! grep -q '^kill -s 9 234$' "${CALLS_FILE}" || fail 'DNS owner cleanup killed an unknown port 53 owner without force'
