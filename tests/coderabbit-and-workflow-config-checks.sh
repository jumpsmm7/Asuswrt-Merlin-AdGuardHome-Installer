...
grouped_shell_regression_step_is_aggregated() {
	awk -v step_name="$2" -v expected_scripts="$3" '
		...
		in_step {
			if ($0 ~ /^[[:space:]]*(command[[:space:]]+)?(\/usr\/bin\/timeout[[:space:]]+([^[:space:]]+[[:space:]]+)*)?busybox[[:space:]]+ash[[:space:]]+tests\/[^[:space:]]*\.sh([[:space:]]|$)/) {
				script = $0
...
for mutation in missing duplicate unguarded moved_named moved_unnamed after_exit unlisted_unguarded unlisted_altered_timeout unlisted_direct unlisted_extra_indent unlisted_command_prefix; do
