...
ip() {
	case "$*" in
		...
		'-o -6 addr list br0 scope global')
			[ -n "${IPV6_FROM_IP:-}" ] && printf '1: br0    inet6 %s/64 scope global dynamic\n' "${IPV6_FROM_IP}"
			;;
		*) return 1 ;;
	esac
}
...
assert_bind_values() {
	...
	[ "${SETUP_DNS_BIND_HOST6:-}" = "${expected_dns6}" ] || fail "${case_name}: expected DNS IPv6 ${expected_dns6}, got ${SETUP_DNS_BIND_HOST6:-empty}"
}
