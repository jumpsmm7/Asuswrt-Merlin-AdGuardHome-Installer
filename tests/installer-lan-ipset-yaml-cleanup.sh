...
EOF_YAML
	...
	FAIL_WRITE_KEY=''
	diff -q "${TMP_ROOT}/config.before-${failed_key}" "${CONF_FILE}" >/dev/null 2>&1 || fail "${failed_key} failure did not restore installer config"
	diff -q "${TMP_ROOT}/working.before-${failed_key}" "${YAML_FILE}" >/dev/null 2>&1 || fail "${failed_key} failure did not restore working YAML"
	diff -q "${TMP_ROOT}/original.before-${failed_key}" "${YAML_ORI}" >/dev/null 2>&1 || fail "${failed_key} failure did not restore original YAML"
...
diff -q "${TMP_ROOT}/working-yaml.before-sync" "${YAML_FILE}" >/dev/null 2>&1 || fail 'failed snapshot synchronization changed the working YAML'
diff -q "${TMP_ROOT}/original-yaml.before-sync" "${YAML_ORI}" >/dev/null 2>&1 || fail 'failed snapshot synchronization changed the original YAML snapshot'
...
! diff -q "${YAML_FILE}" "${YAML_ORI}" >/dev/null 2>&1 || fail 'WAN YAML sync replaced the distinct original snapshot with the working YAML'
...
for flow_yaml in \
	...
	diff -q "${YAML_FILE}" "${YAML_FILE}.before" >/dev/null 2>&1 || fail 'WAN YAML sync changed YAML after rejecting unsupported flow style'
done
