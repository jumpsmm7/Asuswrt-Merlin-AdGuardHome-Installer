...
printf '%s\n' "${INSTALLED_RANGES}" | grep -q '1 9 b q;' || fail 'installed-state menu without a backup does not allow option 9'
printf '%s\n' "${INSTALLED_RANGES}" | grep -q '1 9 b r q;' || fail 'installed-state menu with a backup does not allow option 9'
