...
for status_source in "${INSTALLER_PATH}" AdGuardHome.sh; do
	...
	sed -n '/^status_dnsmasq_handoff_state() {$/,/^}$/p' "${status_source}" |
		sed "s#/tmp/AdGuardHome\.dnsmasq\.handoff#${TEST_ROOT}/dangling-handoff#g; s#/tmp/AdGuardHome\.dnsmasq\.lock#${TEST_ROOT}/missing-lock#g; s#/tmp/AdGuardHome\.dns-handoff/active#${TEST_ROOT}/missing-active#g; s#/tmp/AdGuardHome\.dns-handoff/lock#${TEST_ROOT}/missing-dir-lock#g" >"${STATUS_HELPER}" ||
	...
	rm -f "${TEST_ROOT}/dangling-handoff"
done
