...
* LAN/AP/Bridge mode configures that `firewall-start` hook only when the router has active WAN-interface `SNAT` or `MASQUERADE` state, such as a double-NAT topology. Match NAT state to validated `wan0`/`wan1` interface values. Source selectors (`-s` or `--source`) are valid qualifiers and must not disqualify a rule; negated output-interface matches, rules scoped through `-i` or `--in-interface`, and matching text contained only in comments remain ineligible so unrelated bridge, VPN, or guest-interface NAT does not qualify.
...
* Treat the installer-managed `dnsmasq-sdn.postconf` hook as supported, persistent SDN integration only when `rc_support` advertises `mtlancfg`. That capability gates adding or retaining supported SDN content; always remove a stale installer-managed entry when integration is disabled or unsupported, while preserving unrelated user content.
...
* Runtime dnsmasq updates must edit a same-filesystem staged copy, propagate every edit and append failure, run topology-aware IPSET refresh against that staged content, and publish the live `/etc/dnsmasq*.conf` only after refresh succeeds. Refresh or publication failure must preserve the live file and remove staged artifacts; publication failure after a successful refresh must compensate IPSET from the unchanged live configuration.
