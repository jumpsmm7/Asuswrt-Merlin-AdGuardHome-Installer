...
Run the analyzer from installer menu option **9**, or call it directly with `sh installer blocklists` or `sh installer unusedblocklists`. The installer can download and run [a fork of `blocklilst_analyzer.py`](https://gist.github.com/jumpsmm7/da69039cbee6eadd3b1d483c07c4bf19) maintained by [@jumpsmm7](https://github.com/jumpsmm7). The AdGuard Home Blocklist Usage Analyzer was originally written by [@graysky2](https://github.com/graysky2), whose [original gist](https://gist.github.com/graysky2/8035291d1bf87b8fe3693668965337e1) remains credited here. The analyzer inspects AdGuardHome data under `${TARG_DIR}/data`, the filter cache under `${TARG_DIR}/data/filters`, and the AdGuardHome query log to report which filter lists had matching blocking-rule hits during the analyzed log window.
