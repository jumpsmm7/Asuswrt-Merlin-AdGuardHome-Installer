...
case "$1" in
  "monitor-start")
    ...
    ;;
  "start"|"restart")
    if [ -z "$(pidof "$PROCS")" ]; then "$SCRIPT_LOC" init-start >/dev/null 2>&1; else start_AdGuardHome; fi
    ;;
  "stop"|"kill")
    if [ -n "$(pidof "$PROCS")" ]; then "$SCRIPT_LOC" services-stop >/dev/null 2>&1; else stop_AdGuardHome; fi
    ;;
  "dnsmasq")
    dnsmasq_params
    ;;
