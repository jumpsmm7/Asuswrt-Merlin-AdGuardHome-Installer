...
case "$1" in
  "monitor-start")
    trap '{ start_monitor & }; start_AdGuardHome' 10
  ...
  "init-start"|"services-stop")
    ...
    if [ "$1" = "services-stop" ]; then trap 'exit' 10; [ -n "$(pidof "S99${PROCS}")" ] && { kill -s -10 "$(pidof "S99${PROCS}")" 2>/dev/null || killall -q -10 "S99${PROCS}" 2>/dev/null; }; fi
    ;;
  *)
