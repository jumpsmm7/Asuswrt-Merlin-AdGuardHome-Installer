...
netcheck() {
  local ALIVE
  if [ "$(/bin/date +"%Y")" -gt 1970 ] || [ "$(nvram get ntp_ready)" -eq "0" ]; then ALIVE="0"; else ALIVE="1"; fi
  if [ "$(ping 1.1.1.1 -c1 -W2 >/dev/null 2>&1; echo $?)" = "0" ]; then ALIVE="0"; else ALIVE="$((ALIVE+1))"; fi
  if [ "$(nslookup google.com 127.0.0.1 >/dev/null 2>&1; echo $?)" = "0" ]; then ALIVE="0"; else ALIVE="$((ALIVE+1))"; fi
  if [ "$(curl -Is  http://www.google.com | head -n 1 >/dev/null 2>&1; echo $?)" = "0" ]; then ALIVE="0"; else ALIVE="$((ALIVE+1))"; fi
  if [ "$(wget -q --spider http://google.com >/dev/null 2>&1; echo $?)" = "0" ]; then ALIVE="0"; else ALIVE="$((ALIVE+1))"; fi
