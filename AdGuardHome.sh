...
case "$1" in
  ...
  "init-start"|"services-stop")
    timezone
    if [ "$1" = "init-start" ]; then { printf "1" > /proc/sys/vm/overcommit_memory; }; trap '{ "$SCRIPT_LOC" monitor-start >/dev/null 2>&1; }; exit 0' 10; start_AdGuardHome; fi
    if [ "$1" = "services-stop" ]; then trap 'exit 0' 10; stop_AdGuardHome; fi
