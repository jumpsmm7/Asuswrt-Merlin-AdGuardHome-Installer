...
adguard_wan_iptables_state_active() {
	...
	for unit in 0 1; do
		for key in ifname gw_ifname pppoe_ifname; do
			...
			if printf '%s\n' "${nat_rules}" | /bin/grep -F -e "-o ${ifname} " | /bin/grep -F -v -e "! -o ${ifname} " | /bin/grep -vE -e '(^|[[:space:]])(![[:space:]]+)?(-s|--source|-i|--in-interface)([[:space:]]|$)' | /bin/grep -qE -e '-j (MASQUERADE|SNAT)([[:space:]]|$)'; then
				return 0
			fi
		done
	done
