...
start_monitor () {
  ...
  EXIT="0"
  while true; do
    if [ "$EXIT" = "1" ]; then logger -st "$NAME" "Stopping Monitor"; trap 1 2 3 10 15; stop_AdGuardHome; break; fi 
...
case "$1" in
  ...
  "init-start"|"services-stop")
    timezone
    if [ "$1" = "init-start" ]; then { printf "1" > /proc/sys/vm/overcommit_memory; }; [ -n "$(pidof "S99${PROCS}" | sed "s/\<$$\>//g;s/^[ \t]*//;s/[ \t]*$//")" ] && { kill -s -10 "$(pidof "S99${PROCS}" | sed "s/\<$$\>//g;s/^[ \t]*//;s/[ \t]*$//")"; }; { "$SCRIPT_LOC" monitor-start >/dev/null 2>&1; }; start_AdGuardHome; fi
    if [ "$1" = "services-stop" ]; then trap 'exit 0' 10; { kill -s -10 "$(pidof "S99${PROCS}")" 2>/dev/null || killall -q -10 "S99${PROCS}" 2>/dev/null; }; fi
    ;;
  *)
